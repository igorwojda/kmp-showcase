...
/**
 * Base ViewModel that owns a FlowMVI [store] and adds typed entry points for the iOS consumer.
 *
 ...
 * - **Android consumer** (Kotlin): use [store] directly, e.g. `store.subscribe { … }` and `store.intent(…)`.
 */
abstract class StoreViewModel<S : MVIState, I : MVIIntent, A : MVIAction> :
     ...
     */
    val actions: Flow<A> get() = channelFlow {
